...
starlab::PointSoup pps,pps2;
starlab::SphereSoup spheres;
starlab::VectorSoup vs;
starlab::LineSegments lseg;
...
bool nurbs_plugin::mousePressEvent(QMouseEvent * event)
{
...
NURBS::NURBSRectangled nurbs_plugin::surfaceFit( SurfaceMeshModel * part )
{
	...
	if( false )
	{
        ...
        //drawArea()->addRenderObject( new starlab::LineSegments(bf.ls) );
		...
		if(bf.corners.size())
		{
            starlab::PointSoup * ppp = new starlab::PointSoup(20);
		...
		{
            starlab::PointSoup * ppe = new starlab::PointSoup(10);
...
//	if(!cp.size()) return NURBS::NURBSRectangled::createSheet(Vector3d(0.0,0.0,0.0),Vector3d(0.01));
	if (!cp.size()) return NURBS::NURBSRectangled::createSheet(0.01, 0.01, Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0));
