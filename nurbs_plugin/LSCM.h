...
struct LSCM
{
	LSCM( SurfaceMesh::Model * useMesh )
	{
		...
		this->tex_coord = mesh->add_vertex_property<Vec2f>("v:texture", Vec2f(0, 0));
	...
	static void project_triangle(const Vector3d& p0, const Vector3d& p1, const Vector3d& p2, Vec2f& z0, Vec2f& z1, Vec2f& z2) 
	{
		Vector3d X = (p1 - p0).normalized();
		Vector3d Z = (cross<Vector3d>(X, p2 - p0)).normalized();
		...
		double x1 = (p1 - O).norm() ;
		double y1 = 0 ;
		double x2 = dot<Vector3d>(p2 - O, X);
		double y2 = dot<Vector3d>(p2 - O, Y);
	...
	void get_border_extrema( Halfedge h, const Vector3d& center, const Vector3d& V, Vertex& vx_min, Vertex& vx_max) 
	{
		...
		do {
			Vector3d p = points[mesh->to_vertex(cur)];
			normalize_uv(p) ;
			double v = dot<Vector3d>(p - center, V);
		...
		tex_coord[vx_min] = Vec2f(0,0);
		tex_coord[vx_max] = Vec2f(0,1);
	...
	// Output
	SurfaceMesh::Model::Vertex_property<Vec2f> tex_coord;
