...
Sampler::Sampler(SurfaceMesh::Model * srcMesh, SamplingMethod samplingMethod)
{
	...
	if( method == RANDOM_BARYCENTRIC_AREA || method == RANDOM_BARYCENTRIC_WEIGHTED)
	{
        ...
        interval = std::vector<WeightFace>(mesh->n_faces() + 1);
        interval[0] = WeightFace(0.0, Surface_mesh::Face(0));
		int i = 0;

		// Compute mesh area in a cumulative manner
        foreach(Face f, mesh->faces())
		{
            interval[f.idx()+1] = WeightFace(interval[i].weight + fprobability[f], f);
			i++;
		}
	}
...
QVector<SamplePoint> Sampler::getSamples(int numberSamples, double weight)
{
	...
	if (method == FACE_CENTER_ALL)
	{
		...
		{
			samples << SamplePoint(fcenter[f], fnormal[f], 0, f.idx(), 1 / 3.0, 1 / 3.0);
		}
	}
	else
	{
