...
Q_DECLARE_METATYPE(ThreadsGraph::Sheet*)
...
// Helper functions
RMF Synthesizer::consistentFrame(ThreadsGraph::Curve * curve, Array1D_Vector4d & coords)
{
...
QVector<ParameterCoord> Synthesizer::genPointCoordsCurve(ThreadsGraph::Curve * curve, const std::vector<Vector3f> & points, const std::vector<Vector3f> & normals)
{
...
QVector<ParameterCoord> Synthesizer::genPointCoordsSheet(ThreadsGraph::Sheet * sheet, const std::vector<Vector3f> & points, const std::vector<Vector3f> & normals)
{
	QVector<ParameterCoord> samples(points.size());
...
QVector<ParameterCoord> Synthesizer::genFeatureCoords(ThreadsGraph::Node * node)
{
	...
	if (node->type() == ThreadsGraph::CURVE)
		return genPointCoordsCurve((ThreadsGraph::Curve*)node, meshPoints, meshNormals);
	else
		return genPointCoordsSheet((ThreadsGraph::Sheet*)node, meshPoints, meshNormals);
}

QVector<ParameterCoord> Synthesizer::genEdgeCoords(ThreadsGraph::Node * node)
{
	SurfaceMesh::Model * model = node->property["mesh"].value< QSharedPointer<SurfaceMeshModel> >().data();
	...
	if (node->type() == ThreadsGraph::CURVE)
		return genPointCoordsCurve((ThreadsGraph::Curve*)node, samplePointsF, sampleNormalsF);
	else
		return genPointCoordsSheet((ThreadsGraph::Sheet*)node, samplePointsF, sampleNormalsF);
}
...
QVector<ParameterCoord> Synthesizer::genRemeshCoords(ThreadsGraph::Node * node)
{
	SurfaceMesh::Model * model = node->property["mesh"].value< QSharedPointer<SurfaceMeshModel> >().data();
	...
	if (node->type() == ThreadsGraph::CURVE)
		return genPointCoordsCurve((ThreadsGraph::Curve*)node, samplePoints, sampleNormals);
	else
		return genPointCoordsSheet((ThreadsGraph::Sheet*)node, samplePoints, sampleNormals);
}

QVector<ParameterCoord> Synthesizer::genUniformTrisCoords(ThreadsGraph::Node * node)
{
	SurfaceMesh::Model * model = node->property["mesh"].value< QSharedPointer<SurfaceMeshModel> >().data();
...
QVector<ParameterCoord> Synthesizer::genSampleCoordsCurve(ThreadsGraph::Curve * curve, int s)
{
...
//	if (!curve->property.contains("original_sheet"))
	...
	else
	{
		...
		ThreadsGraph::Sheet* originalSheet = curve->property["original_sheet"].value<ThreadsGraph::Sheet*>();
...
QVector<ParameterCoord> Synthesizer::genSampleCoordsSheet(ThreadsGraph::Sheet * sheet, int s)
{
	QVector<ParameterCoord> samples;
...
void Synthesizer::sampleGeometryCurve(QVector<ParameterCoord> samples, ThreadsGraph::Curve * curve, QVector<float> &offsets, QVector<Vec2f> &normals)
{
...
void Synthesizer::sampleGeometrySheet(QVector<ParameterCoord> samples, ThreadsGraph::Sheet * sheet, QVector<float> &offsets, QVector<Vec2f> &normals)
{
...
// Co-sampling
void Synthesizer::prepareSynthesizeCurve(ThreadsGraph::Curve * curve1, ThreadsGraph::Curve * curve2, int s, SynthData & output)
{
...
void Synthesizer::prepareSynthesizeSheet(ThreadsGraph::Sheet * sheet1, ThreadsGraph::Sheet * sheet2, int s, SynthData & output)
{
...
/// RECONSTRUCTION
void Synthesizer::reconstructGeometryCurve(ThreadsGraph::Curve * base_curve, const QVector<ParameterCoord> & in_samples, const QVector<float> &in_offsets,
...
void Synthesizer::reconstructGeometrySheet(ThreadsGraph::Sheet * base_sheet, const QVector<ParameterCoord> &in_samples, const QVector<float> &in_offsets,
	const QVector<Vec2f> &in_normals, QVector<Vector3f> &out_points, QVector<Vector3f> &out_normals, bool isApprox )
{
	out_points.clear();
	out_points.resize(in_samples.size());
...
/// BLENDING
void Synthesizer::blendCurveBases(ThreadsGraph::Curve * curve1, ThreadsGraph::Curve * curve2, float alpha)
{
...
void Synthesizer::blendSheetBases(ThreadsGraph::Sheet * sheet1, ThreadsGraph::Sheet * sheet2, float alpha)
{
    sheet1 = sheet1;
    sheet2 = sheet2;
    alpha = alpha;
...
void Synthesizer::blendGeometryCurves(ThreadsGraph::Curve * curve, float alpha, const SynthData & data, QVector<Vector3f> &points, QVector<Vector3f> &normals, bool isApprox)
{
...
void Synthesizer::blendGeometrySheets(ThreadsGraph::Sheet * sheet, float alpha, const SynthData & data, QVector<Vector3f> &points, QVector<Vector3f> &normals, bool isApprox)
{
	alpha = qMax(0.0f, qMin(alpha, 1.0f));
	QVector<ParameterCoord> samples = data["node1"]["samples"].value< QVector<ParameterCoord> >();
	QVector<float> offsets1 = data["node1"]["offsets"].value< QVector<float> >();
...
void Synthesizer::saveSynthesisData(ThreadsGraph::Node *node, QString prefix, SynthData & input)
{
	if(!node) return;
...
int Synthesizer::loadSynthesisData(ThreadsGraph::Node *node, QString prefix, SynthData & output)
{
	if(!node) return 0;
