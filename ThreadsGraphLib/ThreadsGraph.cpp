...
Graph::Graph(const Graph & other)
{
	...
	foreach(Link * e, other.edges)
	{
		...
		ThreadsGraph::Link * newEdge = this->addEdge(n1, n2, e->coord[0], e->coord[1], e->id);
...
QVector<int> ThreadsGraph::Graph::getEdgeIDs(QVector<Link*> forEdges)
{
	QVector<int> result;
	...
	foreach(Node * n, nodes)
	{
		...
		if (property["showTasks"].toBool())
		{
			...
			if (n->property["isActive"].toBool())
			{
				...
				// For edges
				foreach(ThreadsGraph::Link * l, edges)
				{
...
void Graph::exportAsOBJ(QString filename)
{
	...
	foreach(ThreadsGraph::Node * n, nodes)
	{
		if (!n->property.contains("mesh")) continue;
...
{
	ThreadsGraph::Node * n = getNode(oldNodeID);
	if (!n) return;
...
bool Graph::hasDoubleEdges(QString nodeID)
{
	...
	if (n == nullptr || links.isEmpty() || n->type() == ThreadsGraph::SHEET) return false;
...
void Graph::cutNode(QString nodeID, int cutCount)
{
	...
	if (n->type() == ThreadsGraph::SHEET)
		return; // no support for sheets for now..
	else
	{
		ThreadsGraph::Curve * c = (ThreadsGraph::Curve *) n;
		...
		// Add nodes
		QColor groupColor = starlab::qRandomColor2();
		QVector<ThreadsGraph::Node*> newNodes;
		for (int i = 0; i < cutCount; i++)
		{
			QString cutNodeID = QString("%1Cut%2").arg(n->id).arg(i);
			auto cutNode = addNode(new ThreadsGraph::Curve(NURBS::NURBSCurved::createCurveFromPoints(cutsGeometry[i]), cutNodeID, groupColor));
...
void Graph::joinNodes(QStringList nodeIDs)
{
	...
	if (nodeType == ThreadsGraph::SHEET)
	{
		return; // no support for sheets for now..
	}
	else
		// curve-chain joins
	{
		...
		for (auto nid : nodeIDs)
		{
			auto n = (ThreadsGraph::Curve*) getNode(nid);
		...
		auto joinedNode = addNode(new ThreadsGraph::Curve(NURBS::NURBSCurved::createCurveFromPoints(new_curve_pts), newNodeID));
