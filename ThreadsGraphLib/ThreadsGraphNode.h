...
#include "ThreadsGraphGlobal.h"
#include "ThreadsGraphLink.h"
...
namespace ThreadsGraph{
	struct Node
	{
		...
		virtual void equalizeControlPoints(ThreadsGraph::Node * other) = 0;
		...
		inline Vector4d minCoord(){ return Vector4d(0, 0, 0, 0); }
		inline Vector4d maxCoord(){ return Vector4d(1, 1, 0, 0); }


		virtual Array1D_Vector3 discretizedAsCurve(Scalar resolution) = 0;
		...
		Array1D_Vector3 geometricDiff(ThreadsGraph::Node * other){
		...
		Scalar distortion(ThreadsGraph::Node * other){
			if (!other) return 0;
