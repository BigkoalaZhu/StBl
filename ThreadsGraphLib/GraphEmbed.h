...
struct GraphEmbed
{
	static void embed(ThreadsGraph::Graph * graph)
	{
		// Get nodes centers
		QMap<int, Node*> nmap;
		...
		for (int i = 0; i < N; i++)
		{
			...
			nmap[i]->vis_property[NODE_CENTER].setValue(RotateFromTo(plane_normal, Vector3(0, 0, 1), p, plane_center));
		}

		graph->property["embeded2D"] = true;
	}

	static void sphereRandomEmbed(ThreadsGraph::Graph * graph)
	{
		// Get nodes centers
		QMap<int, Node*> nmap;
		std::vector<Vector3> centers;
	...
	static void circleEmbed(ThreadsGraph::Graph * graph)
	{
