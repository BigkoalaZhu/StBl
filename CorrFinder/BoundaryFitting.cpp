...
void BoundaryFitting::doFit()
{
	...
	// Cluster boundary
	{
		...
		for (int i = 0; i < (int)boundry.size(); i++)
		{
			...
			Eigen::Vector3d edge = (points[boundry[(i + 1) % boundry.size()]] - points[v]).normalized();

			Eigen::Vector3d prev = vdirection[v];
		...
		// Shortest paths
		{
			...
			//if( false )
			{
				std::vector<Eigen::Vector3d> startPath, endPath;
				...
				std::vector< std::vector<Eigen::Vector3d> > paths = geodesicPaths(startPath, endPath, segments);
				foreach(std::vector<Eigen::Vector3d> path, paths)
				{
...
std::vector< std::vector<Eigen::Vector3d> > BoundaryFitting::geodesicPaths(std::vector<Eigen::Vector3d> fromPoints, std::vector<Eigen::Vector3d> toPoints, int segments)
{
	...
	QSet<Vertex> vertSet;
	foreach(Eigen::Vector3d p, toPoints) vertVector.push_back(Vertex(tree.closest(p)));
	...
	for (int sid = 0; sid < (int)fromPoints.size(); sid++)
	{
		...
		while (true)
		{
			...
			std::vector<Eigen::Vector3d> vp = trianglePoints(f);
...
std::vector<Eigen::Vector3d> BoundaryFitting::geodesicPath(Eigen::Vector3d fromPoint, Eigen::Vector3d toPoint)
{
	return geodesicPaths(std::vector<Eigen::Vector3d>(1, fromPoint), std::vector<Eigen::Vector3d>(1, toPoint)).front();
}

SurfaceMesh::Face BoundaryFitting::getBestFace(Eigen::Vector3d & point, Vertex guess)
{
	Face best_face;
	...
	std::vector<Eigen::Vector3d> vp = trianglePoints(best_face);
	Eigen::Vector3d faceCenter = (vp[0] + vp[1] + vp[2]) / 3.0;
	Eigen::Vector3d delta = point - faceCenter;
	point = faceCenter + (delta * 0.99);

	return best_face;
}

SurfaceMesh::Halfedge BoundaryFitting::getBestEdge(Eigen::Vector3d prevPoint, Eigen::Vector3d direction, Face f, double & bestTime)
{
...
std::vector<Eigen::Vector3d> BoundaryFitting::trianglePoints(Face f)
{
...
void BoundaryFitting::gradientFaces(ScalarVertexProperty & functionVal, bool isSmooth, bool isNormalizeNegateGradient)
{
	...
	if (isSmooth)
	{
		...
		{
			std::vector<Eigen::Vector3d> newGrad(part->n_faces(), Vector3(0, 0, 0));
			std::vector<Eigen::Vector3d> avgNormal(part->n_faces(), Vector3(0, 0, 0));
