...
void HausdorffImageSimplify::extractSets()
{
	...
	{
		Eigen::VectorXd tmp = Eigen::VectorXd::Zero(m_grayImage.cols);
		for(int j=0;j<m_grayImage.cols;j++)
			tmp[j] = m_grayImage.at<uchar>(i, j);
...
void HausdorffImageSimplify::computeRCDists()
{
	...
	/*	if(i==310)	
			cout<<"debug\n";*/
		if(m_rowset[i].sum()!=0&&m_rowset[i-1].sum()!=0)
		{
...
void HausdorffImageSimplify::saveSegmentImg(string name)
{
	...
	for(int i=0;i<m_nodes.size();i++)
	{
		...
		for(int j=start;j<=end;j++)
			for(int k=0;k<rimg.rows;k++)
			{
			double c = m_grayImage.at<UCHAR>(j, k);
...
void HausdorffImageSimplify::saveNodeInfoSpecific(string filename)
{
	...
	{
		nodefile << m_nodes[i].start << '\t' << m_nodes[i].end << '\t' << m_nodes[i].height << '\t' << m_nodes[i].line.size() << endl;
		double maxP = m_nodes[i].line.maxCoeff();
		for (int j = 0; j<m_nodes[i].line.size(); j++)
			nodefile << m_nodes[i].line[j] / maxP << ' ';
		nodefile << endl;
	}
	nodefile.close();
}
...
vector<HausdorffNode> HausdorffImageSimplify::loadNodeInfo(string nodefile)
{
	...
	while(!nodein.eof()){
		...
		Eigen::VectorXd pts = Eigen::VectorXd::Zero(numPtInline);
